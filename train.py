...
import wandb  # 导入Python模块"Wandb"（也称为"Weights and Biases"）
from evaluate import evaluate  # 从一个名为 "evaluate" 的模块导入一个名为 "evaluate" 的函数。
# from unet import UNet  # 从名为"unet"的模块中导入了一个名为"UNet"的类
# from bubbliiiing.unet import Unet  # 从名为"unet"的模块中导入了一个名为"UNet"的类
# 导入两个自定义数据集类BasicDataset和CarvanaDataset
from code.bubbliiiing.hrnet_cbam_out_building_rgb3 import HRnet
...
from focal_loss import FocalBCELoss
...
dir_checkpoint = Path(
    './runs/Ablation_study/final_epoch50_5')
...
def train_model(
    ...
    # n_val = int(len(dataset) * val_percent) #代码计算出验证集(val_set)的大小，即数据集长度乘以给定的验证集占比(val_percent)，然后将其转换为整数(n_val)。
    # n_train = len(dataset) - n_val #计算训练集(train_set)的大小，即数据集长度减去验证集大小(n_val)。
    # train_set, val_set = random_split(
    #     dataset, [n_train, n_val], generator=torch.Generator().manual_seed(0))
    # 通过PyTorch中的random_split函数，基于生成器(torch.Generator().manual_seed(0))随机将数据集(dataset)分成训练集(train_set)和验证集(val_set)，并返回两个新的数据集对象。其中，random_split函数的第一个参数是要划分的数据集，第二个参数是划分后每个数据集的大小列表，这里为[n_train, n_val]。
...
# batch: 一个批次——把数据集1分成多个批次。批次可以理解为一组具有相同属性的数据集合。
    ...
    for epoch in range(1, epochs + 1):  # 用来控制模型训练的循环次数
        ...
        with tqdm(total=len(train_loader), desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            ...
            # Evaluation round
            # division_step = (n_train // (5 * batch_size)) #控制了训练数据的分组方式。将训练集分成多少个较小的子集，以便在每个epoch（即一次完整的训练循环）期间能够有效地使用内存。
            # if division_step > 0:
            #     if global_step % division_step == 0:
            #         histograms = {}
            #         for tag, value in model.named_parameters():
            #             tag = tag.replace('/', '.') #使用了Python中的for循环来迭代模型(model)中所有被命名的参数(named_parameters())。对于每个参数，它会将其名称(tag)中所有斜杠(/)替换为点号(.)，以便更好地表示参数层次结构。然后，它将新的标签(tag)和该参数的值(value)保存在一个元组(tuple)中
            ...
            # val_score = evaluate(model, val_loader, device, amp) #验证模型性能：使用evaluate函数对模型在验证集（val_loader）上的性能进行评估，将评估结果保存在val_score变量中。
            # scheduler.step(train_loss) #更新学习率：使用scheduler.step()方法，根据上一步中得到的val_score更新学习率。scheduler是一个学习率调度器，它可以根据指定的规则自动调整学习率，以提高模型的性能和收敛速度。通常情况下，如果验证集上的性能没有提升，则会降低学习率；反之则会增加学习率。
            # v_score.append(val_score)
            # if dist.get_rank() == 0:
            #     logging.info(
            #         'Validation Dice score: {}'.format(val_score)) #记录并打印出验证得分的信息
            # try:
            #     experiment.log({
...
# 保存模型
        # 在验证集上循环，计算验证集上的损失（validation_loss）
        # 在每个 epoch 结束后进行验证，计算 validation_loss
        # 如果 validation_loss 更低于 best_validation_loss，则更新 best_validation_loss 并保存 checkpoint

        # if val_score > best_val_score:
        #     best_val_score = val_score  # 更新 best_validation_loss
        # #if save_checkpoint: #如果需要保存检查点
        #     Path(dir_checkpoint).mkdir(parents=True, exist_ok=True) # 创建指定目录（如果不存在），parents=True 表示可以递归创建父级目录，exist_ok=True 表示目录已存在则不会抛出异常。
        #     state_dict = model.state_dict() #获取当前模型的参数字典 state_dict.state_dict 是一个字典对象，包含了神经网络模型中所有的可训练参数和非可训练参数的键值对
        #     state_dict['mask_values'] = dataset.mask_values #将dataset中的mask_values属性的值赋给state_dict字典中的mask_values键。
        #     torch.save(state_dict, str(dir_checkpoint /
        #             'checkpoint_epoch{}.pth'.format(epoch))) #将 state_dict 对象保存到指定的文件路径中。
        #     logging.info(f'Checkpoint {epoch} saved!') #记录程序运行时出现的信息.f-string 格式化表达式被用来将变量 epoch 的值插入到字符串中去。最终输出的日志信息为 “Checkpoint 5 saved！

        # if save_checkpoint and epoch == epochs:  # 只在最后一个epoch保存检查点
...
if __name__ == '__main__':  # 运行脚本时从此处开始运行
   ...
   # 打印日志
    ...
    #                  f'\t{"Bilinear" if model.bilinear else "Transposed conv"} upscaling')  # 输出一些关于神经网络的信息
    ...
    # try:
    #     #调用 `train_model` 函数，对模型进行训练。函数中传入了多个参数，包括模型、训练轮数、批次大小、学习率、设备类型、图片缩放比例、验证集占比和是否启用自动混合精度（AMP）等。
    #     train_model(
    #         model=model,
    #         epochs=args.epochs,
    ...
    #     )
    # except:
    #     #如果在训练过程中出现内存溢出（OutOfMemoryError），则捕获异常并执行以下操作
    #     #输出错误日志信息，提示用户开启检查点以降低内存使用，但会导致训练速度变慢
    #     logging.error('Detected OutOfMemoryError! '
    #                   'Enabling checkpointing to reduce memory usage, but this slows down training. '
    #                   'Consider enabling AMP (--amp) for fast and memory efficient training')
    #     torch.cuda.empty_cache() #清空CUDA缓存以释放内存
    #     # model.use_checkpointing()
    #     #再次调用 `train_model` 函数重新训练模型，参数与前面相同
    #     train_model(
    #         model=model,
    #         epochs=args.epochs,
    #         batch_size=args.batch_size,
    ...
    # total_receptive_field = UNet.calculate_total_receptive_field(model)
